...
?>
    ...
    <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
      <p class="submit">
        <input type="submit" class="button-primary"    name="button_update_bchwc_settings"        value="<?php _e('Save Changes') ?>"             />
        <input type="submit" class="button-secondary"  style="color:red;" name="button_reset_partial_bchwc_settings" value="<?php _e('Reset settings') ?>" onClick="return confirm('Are you sure you want to reset settings on this page?');" />
      </p>
      <table class="form-table">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="hidden" name="delete_db_tables_on_uninstall" value="0" /><input type="checkbox" name="delete_db_tables_on_uninstall" value="1" <?php if ($bchwc_settings['delete_db_tables_on_uninstall']) echo 'checked="checked"'; ?> />
        ...
        <tr valign="top">
          ...
          <td>
            <select name="service_provider" class="select ">
              <option <?php if ($bchwc_settings['service_provider'] == 'electrum_wallet') echo 'selected="selected"'; ?> value="electrum_wallet">Your own Electron Cash wallet</option>
              <option <?php if ($bchwc_settings['service_provider'] == 'blockchain_info') echo 'selected="selected"'; ?> value="blockchain_info">Blockchain.info API (DOES NOT WORK FOR BITCOIN CASH! use Electron Cash instead)</option>
            </select>
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <textarea style="width:75%;" name="electrum_mpk_saved"><?php echo $bchwc_settings['electrum_mpk_saved']; ?></textarea>
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="confs_num" value="<?php echo $bchwc_settings['confs_num']; ?>" size="4" />
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <select name="exchange_rate_type" class="select ">
              <option <?php if ($bchwc_settings['exchange_rate_type'] == 'vwap') echo 'selected="selected"'; ?> value="vwap">Weighted Average</option>
              <option <?php if ($bchwc_settings['exchange_rate_type'] == 'realtime') echo 'selected="selected"'; ?> value="realtime">Real Time</option>
              <option <?php if ($bchwc_settings['exchange_rate_type'] == 'bestrate') echo 'selected="selected"'; ?> value="bestrate">Most profitable</option>
            </select>
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="exchange_multiplier" value="<?php echo $bchwc_settings['exchange_multiplier']; ?>" size="4" />
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="hidden" name="autocomplete_paid_orders" value="0" /><input type="checkbox" name="autocomplete_paid_orders" value="1" <?php if ($bchwc_settings['autocomplete_paid_orders']) echo 'checked="checked"'; ?> />
        ...
        <tr valign="top">
            ...
            <td>
              <select name="enable_soft_cron_job" class="select ">
                <option <?php if ($bchwc_settings['enable_soft_cron_job'] == '1') echo 'selected="selected"'; ?> value="1">Soft Cron (Wordpress-driven)</option>
                <option <?php if ($bchwc_settings['enable_soft_cron_job'] != '1') echo 'selected="selected"'; ?> value="0">Hard Cron (Cpanel-driven)</option>
              </select>
              <p class="description">
                <?php if ($bchwc_settings['enable_soft_cron_job'] != '1') echo '<p style="background-color:#FFC;color:#2A2;"><b>NOTE</b>: Hard Cron job is enabled: make sure to follow instructions below to enable hard cron job at your hosting panel.</p>'; ?>
...
<h3 style="text-align:center;color:#090;">Get the <a href="<?php echo BWWC__GetProUrl(); ?>"><b>PRO version</b></a></h3>
      ...
      <p class="submit">
          <input type="submit" class="button-primary"    name="button_update_bchwc_settings"        value="{$value_save_changes}"             />
          <input type="submit" class="button-secondary"  style="color:red;" name="button_reset_partial_bchwc_settings" value="{$value_reset_settings}" onClick="return confirm('Are you sure you want to reset settings on this page?');" />
      </p>
  </form>
TTT;
  echo $html_out;
